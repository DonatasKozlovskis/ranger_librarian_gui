#include "../include/ranger_librarian_gui/label_reader.hpp"
...
void LabelReader::prepareUserImage(cv::Mat &inputFrame) {
    ...
    cv::cvtColor(frameFlipped, inputFrame, cv::COLOR_RGB2GRAY);
    // blur
     cv::blur(inputFrame, inputFrame, cv::Size(blur_kernel, blur_kernel));
    // convert back to colour image variable
     cv::cvtColor(inputFrame, inputFrame, cv::COLOR_GRAY2RGB);
    ...
    // draw the reading boxes on original frame
     cv::rectangle(inputFrame, labelRegionSquare, cv::Scalar(255,0,0), 2);
     cv::rectangle(inputFrame, labelRegionRect, cv::Scalar(255,0,0), 2);
